...
public class Label implements CDrawable {
	...
	@Override public void draw(PGraphics graphics) {
		if (isVisible) {
			_myFontLabel.adjust(graphics, this);
			draw(graphics, 0, 0);
		}
	}
	...
	public void draw(PGraphics graphics, int theX, int theY) {
		if (isVisible) {
			graphics.pushMatrix();
			graphics.translate(_myControllerStyle.marginLeft, _myControllerStyle.marginTop);
			...
			if (isColorBackground) {
				...
				graphics.fill(_myColorBackground);
				graphics.rect(0, 1, ww, hh);
			}
			graphics.translate(_myControllerStyle.paddingLeft, _myControllerStyle.paddingTop);
			_myFontLabel.draw(graphics, this);
			graphics.popMatrix();
		}
	}
