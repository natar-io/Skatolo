...
public class Textarea extends ControllerGroup<Textarea> implements ControlListener {
	...
	protected boolean inside() {
		return (cp5.getWindow().getPointerX() > position.x + _myParent.absolutePosition.x
				&& cp5.getWindow().getPointerX() < position.x + _myParent.absolutePosition.x + _myWidth
				&& cp5.getWindow().getPointerY() > position.y + _myParent.absolutePosition.y && cp5.getWindow().getPointerY() < position.y
