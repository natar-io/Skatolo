...
import fr.inria.skatolo.ControlFont;
...
/**
 * a textarea can be used to leave notes, it uses the skatolo BitFont to render text. Scrollbars
 ...
 * 
 * @example controllers/skatolotextarea
 */
public class Textarea extends ControllerGroup<Textarea> implements ControlListener {
	 ...
	 * @param theName
	 */
	public Textarea(Skatolo theskatolo, String theName) {
		this(theskatolo, theskatolo.getDefaultTab(), theName, "", 0, 0, 199, 99);
	 ...
	 * @param theH int
	 */
	public Textarea(Skatolo theskatolo, ControllerGroup<?> theGroup, String theName, String theText, int theX,
			int theY, int theW, int theH) {
		super(theskatolo, theGroup, theName, theX, theY);
		_myWidth = theW;
		_myHeight = theH;
		_myText = theText;
		setup();
	...
	private void setup() {
		_myValueLabel = new Label(skatolo, _myText);
		...
		_myScrollbar = new Slider(skatolo, _myParent, getName() + "Scroller", 0, 1, 1, _myWidth - 5, 0, 5, _myHeight);
		_myScrollbar.init();
	...
	protected boolean inside() {
		return (skatolo.getWindow().getPointerX() > position.x + _myParent.absolutePosition.x
				&& skatolo.getWindow().getPointerX() < position.x + _myParent.absolutePosition.x + _myWidth
				&& skatolo.getWindow().getPointerY() > position.y + _myParent.absolutePosition.y && skatolo.getWindow().getPointerY() < position.y
