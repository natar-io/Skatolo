...
/**
 * @exmaple controllers/skatolodropdownList
 */
public class DropdownList extends ControlGroup<DropdownList> {
	...
	public DropdownList(Skatolo theskatolo, ControllerGroup<?> theGroup, String theName, int theX, int theY, int theW, int theH) {
		...
		_myScrollbar = new Slider(skatolo, _myParent, theName + "Scroller", 0, 1, 1, _myWidth - _myScrollbarWidth, 0, _myScrollbarWidth,
				_myBackgroundHeight);
	...
	@Override public DropdownList setValue(float theValue) {
		for (ListBoxItem l : items) {
			if ((l.value == theValue)) {
				_myValue = l.value;
				setLabel(l.name);
				skatolo.getControlBroadcaster().broadcast(new ControlEvent(this), SkatoloConstants.FLOAT);
				break;
			}
		}
		return this;
	...
	@Override public DropdownList updateInternalEvents(PApplet theApplet) {
		...
		boolean yy = skatolo.getWindow().getPointerY() > getAbsolutePosition().y - 1
				&& skatolo.getWindow().getPointerY() < getAbsolutePosition().y + _myBackgroundHeight;
	...
	protected DropdownList addListButton(int theNum) {
		for (int i = 0; (i < theNum) && (buttons.size() < maxButtons); i++) {
			...
			Button b = new Button(skatolo, (ControllerGroup<?>) this, _myName + "Button" + index, index, 0, index * (_myItemHeight + spacing),
	...
	public void keyEvent(final KeyEvent theEvent) {
		...
		boolean b = (getWindow().getPointerX() > x && getWindow().getPointerX() < (x + _myWidth) && getWindow().getPointerY() > (y - getBarHeight()) && getWindow().getPointerY() < y
