 ...
 *
 *
 * @example controllers/skatolonumberbox
...
public class Numberbox extends Controller<Numberbox> {
    ...
    public Numberbox(Skatolo theskatolo, Tab theParent, String theName, float theDefaultValue, int theX, int theY, int theWidth, int theHeight) {
        ...
        _myValueLabel = new Label(skatolo, "" + _myValue, theWidth, 12, color.getValueLabel());
        ...
        if (isActive) {
            if (!skatolo.isAltDown()) {
                if (_myNumberCount == VERTICAL) {
                    setValue(_myValue + (controlWindow.getPointerY() - controlWindow.getPointerPrevY()) * _myMultiplier);
                } else {
                    setValue(_myValue + (controlWindow.getPointerX() - controlWindow.getPointerPrevX()) * _myMultiplier);
                }
            }
        }
        return this;
     ...
     * (non-Javadoc)
     * 
     * @see skatolo.Controller#mousePressed()
     */
    @Override
     ...
     * (non-Javadoc)
     * 
     * @see skatolo.Controller#mouseReleased()
     */
    @Override
     ...
     * (non-Javadoc)
     * 
     * @see skatolo.Controller#mouseReleasedOutside()
     */
    @Override
    ...
    /**
     * @see skatolo.Numberbox#setScrollSensitivity(float)
     *
     * @param theValue
     * @return
     */
