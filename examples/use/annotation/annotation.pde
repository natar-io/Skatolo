 ...
 * from Karsten Schmidt's (toxi) skatolomagic library. 
 * Loving the simplicity of annotations and how it is aplied with skatolomagic,
 * i had to include it into skatolo and the following example 
 * shows how to use annotations with skatolo. 
 *
...
void setup() {
  ...
  skatolo.addControllersFor(this);
  
  
  
  
  extraTab = skatolo.addTab("extra");
  ...
  // e.g. variable x
  // /world/x
  skatolo.addControllersFor("world", tc1);

  // set the Position of controllers contained within object tc1
  skatolo.setPosition(10, 150, tc1);
  ...
  skatolo.addControllersFor("hello", tc2)
     .setPosition(200, 150, tc2)
     .moveTo(extraTab, tc2)
     .setColor(col, tc2);
     
  skatolo.getController("s", tc2)
